...
public class GeetestLib {
	...
	protected Boolean debugCode = false;//调试开关，是否输出调试日志
	...
	public static int getGtServerStatusSession(HttpServletRequest request) {
		return (Integer) request.getSession().getAttribute(gt_server_status_session_key);
	}
	...
	public String getSuccessPreProcessRes() {
		return String.format("{\"success\":%s,\"gt\":\"%s\",\"challenge\":\"%s\"}", 1, this.getCaptchaId(), this.getChallengeId());
	}
	...
	public String getGtFrontSource() {
		...
		String frontSource = String.format("<script type=\"text/javascript\" src=\"%s/get.php?" + "gt=%s&challenge=%s", base_path, this.captchaId, this.challengeId);
	...
	public boolean validateRequest(HttpServletRequest request) {
		...
		boolean gtResult = this.validate(request.getParameter(this.fn_geetest_challenge), request.getParameter(this.fn_geetest_validate), request.getParameter(this.fn_geetest_seccode));
