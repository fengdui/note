...
import com.xh.market.framework.filesystem.common.IniFileReader;
...
public class ClientGlobal {
	...
	public static void init(Integer connectTimeout, Integer networkTimeout, String charset, String trackerServers, Integer trackerHttpPort, Boolean antiStealToken, String secretKey)
		...
		g_connect_timeout = connectTimeout * 1000; //millisecond

		if (null == networkTimeout || networkTimeout < 0) {
			networkTimeout = DEFAULT_NETWORK_TIMEOUT;
		}
		g_network_timeout = networkTimeout * 1000; //millisecond
		...
		if (null == trackerHttpPort || trackerHttpPort < 0) {
			trackerHttpPort = 80;
		}
		g_tracker_http_port = trackerHttpPort;
		
		if (null == antiStealToken) {
			antiStealToken = false;
		}
		g_anti_steal_token = antiStealToken;
		if (g_anti_steal_token) {
			g_secret_key = secretKey;
		}
	}
